...
```
Also, we need to install the following packages to start
```{r setup}
require(HaoZheng)
library(stringr)
library(ggplot2)
...
$$H_a: coefficient \neq 0$$
with our significance level
$$ \alpha = 0.05$$
From the output we have, p value is less than 0.05. As a result, we reject the null hypothesis.
