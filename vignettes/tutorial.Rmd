...
lm_1
```
From the output, we can see that the coefficient for gdpPercap has positive value 4.45e-4. For a 1 unit increase in gdpPercap, the lifeExp will increase about 4.45e-04.
...
$$ \alpha = 0.05$$
From the output I have, I reject the null hypothesis and support the alternative hypothes(coefficient is not 0)
...
choose knn = 1, because it has a  training set error of 0.However, I would not
determine only based on CV and I would choose knn = 5, because it provides
more information and more accurate than knn = 1.
...
the opposite. 10 folds also have the lowest sd and k = 2 has the highest, which
is consistent with the plot. Also, a higher number of folds always has higher
accuracy and less mean square error. When we have a relatively small number of
